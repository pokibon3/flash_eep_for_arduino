...
/* Hardware-specific helpers (used by header-declared methods). These
   must be implemented in this .cpp file so that device headers are
   only pulled in here (and not in the public header). */

void FLASH_EEP::flash_wait_busy(void)
{
    while (FLASH->STATR & FLASH_STATR_BSY) {}
}

void FLASH_EEP::flash_clear_flags(void)
{
    // WCH系は STATR に 1 書きでクリア（まとめてクリア）
    FLASH->STATR = 0xFFFFFFFF;
}

void FLASH_EEP::flash_unlock_all(void)
{
    // 通常Flash unlock
